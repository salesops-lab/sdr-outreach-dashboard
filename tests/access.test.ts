...
import { allOwnersInPod, sdrOwnersUnderManager } from "../lib/team/helpers";

const ALL = [...REP_OWNER_IDS];
const TS = configTeamStructure(); // the config-derived structure = current prod scope

// Real owner ids from the config structure.
...
describe("decideScope", () => {
  it("admin/leadership → all tracked + admin flag", () => {
    expect(decideScope("boss@spyne.ai", { role: "admin", team_id: null }, null, ALL, TS)).toMatchObject({ role: "admin", isAdmin: true, defaultOwnerIds: ALL });
    expect(decideScope("l@spyne.ai", { role: "leadership", team_id: null }, null, ALL, TS).isAdmin).toBe(true);
  });

  it("AE pod lead (by email) → the pod's SDRs + AEs", () => {
    const v = decideScope(AE_SAARTHAK, { role: "manager", team_id: "T" }, null, ALL, TS);
    const expected = allOwnersInPod(TS, "saarthak").filter((id) => ALL.includes(id));
  ...
  it("player-coach manager (by owner id) → their team incl. self", () => {
    const v = decideScope("prabhjeet@spyne.ai", null, PRABHJEET, ALL, TS);
    expect(v.role).toBe("manager");
  ...
  it("top manager sees TLs' teams too (recursive subtree)", () => {
    const v = decideScope("vaibhav@spyne.ai", null, VAIBHAV, ALL, TS);
    const shikharTeam = sdrOwnersUnderManager(TS, "shikhar").filter((id) => ALL.includes(id));
    for (const id of shikharTeam) expect(v.defaultOwnerIds).toContain(id); // Vaibhav ⊇ Shikhar's team
    expect(v.defaultOwnerIds.length).toBeGreaterThan(sdrOwnersUnderManager(TS, "shikhar").length);
  });

  it("a TL sees only their own sub-team, not the whole org", () => {
    const v = decideScope("shikhar@spyne.ai", null, SHIKHAR, ALL, TS);
  ...
  it("plain tracked SDR → own data", () => {
    expect(decideScope("sana@spyne.ai", null, SANAMDEEP, ALL, TS)).toMatchObject({ role: "rep", defaultOwnerIds: [SANAMDEEP] });
  });

  it("everyone else → viewer with org-wide default", () => {
    expect(decideScope("cs@spyne.ai", null, null, ALL, TS)).toMatchObject({ role: "viewer", isAdmin: false, defaultOwnerIds: ALL });
  });
});
